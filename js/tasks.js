...
function taskInitiation () {
    ...
    document.getElementById("task-add").addEventListener("click", function () {
        ...
        addForm.innerHTML = `
            ...
            <small id="input-task-error"></small>
        ...
        function validaTarefa(){
            ...
            inputTarefa.addEventListener("keyup",function(){
                if(inputTarefa.value.length>5){
                    ...
                    document.querySelector("#save-task").setAttribute("disabled", true);
                }
            });
         };
    })
    ...
    // itera listeners para os botões de editar tarefa
    function editTaskIteration () {
        let allEditButtons = Array.from(document.querySelectorAll(".edit-button"));
        allEditButtons.forEach(element => {
            element.addEventListener("click", event => openEdit(event.target));
        })
    }
